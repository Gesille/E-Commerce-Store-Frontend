  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (req.user?.role !== "teacher") {
        return next(new ErrorHandler("You are not authorized to upload a course", 403));
      }
      const data = req.body;
...
export const editCourse = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (courseData.createdBy.toString() !== req.user?._id.toString()) {
...
export const getCourseByUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!courseExists) {
        return next(
          new ErrorHandler("أنت غير مؤهل للوصول إلى هذه الدورة", 400)
        );
      }
      ...
      if (!course) {
        return next(new ErrorHandler("الدورة غير موجودة", 404));
      }

      // التحقق من بيانات الدورة
      const content = course?.courseData;
      if (!content) {
        return next(new ErrorHandler("بيانات الدورة غير موجودة", 404));
      }
...
console.log(course?.courseData); // لعرض بيانات الدورة (بما في ذلك الفيديو)
...
export const addQusetion = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
...
export const addReplyToReview = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    if (!req.user || !req.user.role || !["admin", "teacher"].includes(req.user.role)) {
      return next(new ErrorHandler("You do not have permission to reply to reviews", 403));
    }
    try {
...
export const deleteCourse = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!course) {
        return next(new ErrorHandler("Course not founf", 404));
      }
...
export const getAllCommentsForCourses = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const courses = await CourseModel.find().select("reviews");
      if (!courses) {
